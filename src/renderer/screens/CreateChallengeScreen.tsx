...
export function CreateChallengeScreen({ onCreated, onNavigate }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!effectiveDays || isNaN(effectiveDays)) {
      setError("Escolhe uma duração.");
      return;
    }
    if (effectiveDays < 7) {
      setError("Mínimo de 7 dias.");
      return;
    }
    if (reason.trim().length < 10) {
      setError("Escreve pelo menos 10 caracteres no motivo.");
      return;
    }
    setLoading(true);
  ...
  const canSubmit =
    !!effectiveDays && effectiveDays >= 7 && reason.trim().length >= 10;
  ...
  return (
    ...
    >
      ...
      <main style={mainStyle}>
        ...
        <p style={subtext}>
          Define a duração e o teu motivo. Não há volta atrás fácil.
        </p>
        <div
          style={{
            height: "1px",
        ...
        >
          ...
          >
                ...
                <button
                  ...
                  style={{
                    ...
                    background:
                      selectedDays === d && !useCustom
                        ? "var(--green-subtle)"
                        : "transparent", // ← ÚNICA propriedade background
                  }}
                >
                  {d} dias
                </button>
            ...
            <Input
              ...
              style={{
                borderColor: useCustom ? "var(--green)" : "var(--gray-200)",
              }}
            />
            {useCustom && customDays && parseInt(customDays) < 7 && (
              <p style={{ fontSize: "11px", color: "var(--red-muted)" }}>
                Mínimo de 7 dias.
              </p>
            )}
          </div>
