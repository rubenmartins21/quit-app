  ...
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: "2px" }}>
        ...
        <NavItem label="Histórico" isActive={false} disabled />
      </nav>
...
function NavItem({ label, isActive, onClick, disabled }: {
  label: string; isActive: boolean; onClick?: () => void; disabled?: boolean;
}) {
  return (
    <div
      onClick={disabled ? undefined : onClick}
      style={{
        padding: "9px 12px", borderRadius: "var(--radius-sm)", fontSize: "12px", letterSpacing: "0.04em",
        color: isActive ? "var(--white)" : disabled ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.55)",
